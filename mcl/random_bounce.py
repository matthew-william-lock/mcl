...
class RandomBounce(Node):
    ...
    def __init__(self):
        ...
        self.srv = self.create_service(SetBool, '/start_stop_bouncing',
                                       self.start_stop)
        self.timer = self.create_timer(200.0 * MILLISECONDS, self.timer_callback)
    ...
    def laser_scan_callback(self, msg):
        ...
        for laser_range in msg.ranges:
            ...
            if laser_range <= max and laser_range >= min:
                ...
                if (distance <= self.config['object_distance']) and (abs(round(degrees(sensor_angle))) <= self.config['object_angle']):
        ...
        if self.time == 0:
            ...
            if self.initializing:
                self.velocity.linear = Vector3(x=0.0, y=0.0, z=0.0)
                angular_spd = radians(self.config['angular_speed'])
                self.velocity.angular = Vector3(x=0.0, y=0.0, z=angular_spd)
                full_rotation_time = 360 / self.config['angular_speed'] * ONE_SECOND_PERIOD
                self.time = full_rotation_time
                self.initializing = False
            else:
                self.velocity.linear = Vector3(x=self.config['linear_speed'], y=0.0, z=0.0)
